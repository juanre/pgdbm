...
from ..models.project import Project, ProjectCreate, ProjectUpdate, ProjectWithTasks, ProjectStatus, TaskCreate
...
async def create_project(request: Request, project: ProjectCreate):
    ...
    project_obj = await db.create_project(
        project,
...
async def get_project(request: Request, project_id: UUID):
    ...
    tasks = await db.fetch_all(
        "SELECT * FROM {{tables.tasks}} WHERE project_id = $1 ORDER BY created_at DESC",
        project_id
    )
