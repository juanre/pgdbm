...
from shared.models import Order, OrderCreate, OrderWithItems, OrderStatus, MessageResponse
from shared.events import event_bus, EventTypes
from shared.resilience import ServiceClient
from services.user.api import security, JWT_SECRET, JWT_ALGORITHM, HTTPAuthorizationCredentials
...
async def create_order(
    ...
    for item in order_data.items:
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Failed to validate product {item.product_id}: {str(e)}"
            )
...
async def list_orders(
    ...
    orders = await db.list_user_orders(
        user_id=user_id,
        limit=limit,
        offset=offset,
        status=status
