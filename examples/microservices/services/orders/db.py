...
class OrderDatabase(ServiceDatabase):
    ...
    async def create_order(self, user_id: UUID, order_data: OrderCreate, total_amount: Decimal) -> Order:
        """Create a new order."""
    ...
    async def get_order(self, order_id: UUID) -> Optional[OrderWithItems]:
        ...
        # Get order
        order_result = await self.fetch_one(
            "SELECT * FROM {{tables.orders}} WHERE id = $1",
            order_id
        )
        ...
        items_results = await self.fetch_all(
            "SELECT * FROM {{tables.order_items}} WHERE order_id = $1",
            order_id
        )
    ...
    async def update_order_items_pricing(self, order_id: UUID, item_prices: Dict[UUID, Dict[str, Decimal]]) -> None:
