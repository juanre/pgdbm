...
@generate.command()
@click.argument("name")
@click.option("--dual-mode", is_flag=True, default=True, help="Generate dual-mode library (default)")
@click.option("--path", "-p", type=click.Path(path_type=Path), help="Output path")
@click.option("--with-api", is_flag=True, help="Include FastAPI boilerplate")
@click.pass_context
def library(ctx: click.Context, name: str, dual_mode: bool, path: Optional[Path], with_api: bool) -> None:
