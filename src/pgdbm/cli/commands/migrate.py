...
def apply(ctx: click.Context, module: Optional[str], apply_all: bool, dry_run: bool) -> None:
    ...
    if not config:
        ...
        if not migrations_path.exists():
            click.echo("No migrations directory found. Create migrations/ or use --config", err=True)
            ctx.exit(1)
    ...
    async def _apply() -> None:
        ...
        try:
            ...
            for module_name in modules:
                ...
                if not migrations_path.exists():
                    click.echo(f"Warning: Migrations path not found for {module_name}: {migrations_path}")
                    continue
                ...
                if dry_run:
                    ...
                    manager = AsyncMigrationManager(
                        ...
                        schema=module_config.schema_name or env_config.schema_name
                    )
                    ...
                    if result["applied"]:
                        for migration in result["applied"]:
                            click.echo(f"  ✓ {migration['filename']} ({migration['execution_time_ms']}ms)")
...
def _apply_simple_migrations(env_config, migrations_path: Path, dry_run: bool) -> None:
    ...
    async def _status() -> None:
        ...
        try:
            for module_name in modules:
                ...
                click.echo(f"\nModule: {module_name}")
                click.echo(f"  Schema: {module_config.schema_name or env_config.schema_name}")
...
def _show_simple_status(env_config) -> None:
...
@click.option("--path", type=click.Path(path_type=Path), help="Override migrations path")
@click.pass_context
def create(ctx: click.Context, name: str, module: Optional[str], sql: bool, path: Optional[Path]) -> None:
...
def rollback(ctx: click.Context, module: str, version: str, confirm: bool) -> None:
    ...
    if not confirm:
        click.confirm(
            f"⚠️  Rollback module '{module}' to version {version}? This may delete data!",
            abort=True
        )
